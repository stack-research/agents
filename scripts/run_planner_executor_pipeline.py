...
def run_pipeline(
    ...
    if store and run_id:
        save_stage(store, run_id, "planner", plan)
    ...
    if store and run_id:
        save_stage(store, run_id, "executor", execution)

    result = {"plan": plan, "execution": execution, "pipeline_status": "ok"}
    if store and run_id:
...
def main() -> int:
    ...
    if args.state:
        ...
        if not run_id:
            ...
            run_id = datetime.now(timezone.utc).strftime("planner-%Y%m%d-%H%M%S")
